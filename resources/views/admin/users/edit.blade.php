...
<form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <select name="role" id="role" required
                ...
                <option value="sub_affiliate" {{ old('role', $user->role) === 'sub_affiliate' ? 'selected' : '' }}>Sub-Affiliate</option>
            </select>
            @error('role')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div id="parent_id_field" style="display: {{ old('role', $user->role) === 'sub_affiliate' ? 'block' : 'none' }};">
            ...
            <select name="parent_id" id="parent_id"
                ...
                @foreach($users as $parentUser)
                    <option value="{{ $parentUser->id }}" {{ old('parent_id', $user->parent_id) == $parentUser->id ? 'selected' : '' }}>
